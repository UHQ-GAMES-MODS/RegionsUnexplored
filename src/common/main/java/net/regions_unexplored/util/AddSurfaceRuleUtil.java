...
public class AddSurfaceRuleUtil {
    public static void appendSurfaceRule(RegistryAccess registryAccess, ResourceKey<LevelStem> levelStemKey, SurfaceRules.RuleSource ruleSource) {
        ...
        boolean hasRegionsUnexploredBiome = chunkGenerator.getBiomeSource().possibleBiomes().stream().anyMatch(biomeHolder -> biomeHolder.unwrapKey().orElseThrow().location().getNamespace().equals(RegionsUnexplored.MOD_ID));
        if (hasRegionsUnexploredBiome) {
            if (chunkGenerator instanceof NoiseBasedChunkGenerator) {
                ...
                RegionsUnexplored.LOGGER.info(String.format("Ignoring Region Unexplored appended surface rule in dimension: \"%s\" because the chunk generator was not an instance of \"NoiseBasedChunkGenerator\".\nThe chunk generator was an instanceof \"%s\".", levelStemKey.location(), chunkGenerator.getClass().getName()));
            }
        }
