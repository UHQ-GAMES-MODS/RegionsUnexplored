...
public class ConfigManager {
    ...
    public ConfigManager register(String fileName, Class<? extends Config> configClass) {
        ...
        try {
            ...
            RegionsUnexplored.LOGGER.info("Registered annotated config: {}", fileName);
        } catch (InstantiationException | IllegalAccessException | NoSuchMethodException | InvocationTargetException e) {
            RegionsUnexplored.LOGGER.error("Failed to instantiate config class: {}", configClass.getName(), e);
    ...
    public void initialize() {
        ...
        if (!config.getFile().exists()) {
            RegionsUnexplored.LOGGER.info("Config file not found, creating new one: {}", fileName);
            store();
        } else {
            RegionsUnexplored.LOGGER.info("Loading config file: {}", fileName);
            load();
        }
    }
    ...
    private void load() {
        try {
            ...
            for (ConfigValue<?> configValue : configFile.getConfigValues()) {
                ...
                if (loadedValue != null) {
                    ...
                    if (isTypeCompatible(configValue.get(), convertedValue)) {
                        ...
                        RegionsUnexplored.LOGGER.debug("Loaded config value: {} = {} {}",
                                key,
                                convertedValue,
                                configValue.requiresRestart() ? "(requires restart)" : "");
                    } else {
                        RegionsUnexplored.LOGGER.warn("Type mismatch for config key '{}'. Expected: {}, Found: {}. Using default value.",
                                key, configValue.get().getClass().getSimpleName(), convertedValue.getClass().getSimpleName());
                    }
                } else {
                    RegionsUnexplored.LOGGER.debug("Config key '{}' not found in file, using default: {}", key, configValue.get());
                }
            }
            ...
            RegionsUnexplored.LOGGER.error("Failed to load config file: {}", fileName, e);
    ...
    private void store() {
        ...
        try {
            ...
            // Save to file
            config.save();
            RegionsUnexplored.LOGGER.debug("Saved config file: {}", fileName);

        } catch (IOException e) {
            RegionsUnexplored.LOGGER.error("Failed to save config file: {}", fileName, e);
     ...
     */
    public void reload() {
        RegionsUnexplored.LOGGER.info("Reloading config: {}", fileName);
        load();
    }
     ...
     */
    public void save() {
        RegionsUnexplored.LOGGER.info("Saving config: {}", fileName);
        store();
    }
    ...
    private static Path getConfigPath(String fileName) {
        return ConfigHelper.getConfigDirectory().resolve(fileName + ".toml");
    }
