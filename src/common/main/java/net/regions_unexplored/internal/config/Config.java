...
public abstract class Config {
    ...
    public void processAnnotations() {
        ...
        RegionsUnexplored.LOGGER.debug("Processing annotations for config class: {}", clazz.getName());
        ...
        for (Field field : clazz.getDeclaredFields()) {
            ...
            if (!ConfigValue.class.isAssignableFrom(field.getType())) {
                RegionsUnexplored.LOGGER.warn("Field {} has @ConfigOption but is not of type ConfigValue", field.getName());
                continue;
            }
            ...
            try {
                ...
                if (configValue == null) {
                    RegionsUnexplored.LOGGER.warn("Field {} is null, skipping", field.getName());
                    continue;
                }
                ...
                RegionsUnexplored.LOGGER.debug("Registered config value: {} = {}", key, configValue.get());

            } catch (IllegalAccessException e) {
                RegionsUnexplored.LOGGER.error("Failed to access field {}", field.getName(), e);
            }
        }
    }
    ...
    private void processConfigOption(ConfigValue<?> configValue, ConfigOption option, String fieldName) {
        ...
        if (configValue.getKey().isEmpty() || !configValue.getKey().equals(key)) {
            ...
            RegionsUnexplored.LOGGER.debug("Set config key for field {} to '{}'", fieldName, key);
        }
    }
    ...
    private void processRange(ConfigValue<?> configValue, Range range) {
        ...
        if (!Double.isInfinite(min) || !Double.isInfinite(max)) {
            ...
            RegionsUnexplored.LOGGER.debug("Set range for {}: [{}, {}]", configValue.getKey(), min, max);
        }
    }
    ...
    private void processAllowedValues(ConfigValue<?> configValue, AllowedValues allowed) {
        if (allowed.value().length > 0) {
            ...
            RegionsUnexplored.LOGGER.debug("Set allowed values for {}: {}",
