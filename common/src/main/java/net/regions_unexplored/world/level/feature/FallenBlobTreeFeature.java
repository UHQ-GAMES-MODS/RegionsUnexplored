...
public class FallenBlobTreeFeature extends Feature<FallenTreeConfiguration> {
        ...
        while (placeCheck <= height && level.getBlockState(placePos.below()).isFaceSturdy(level, placePos.below(), Direction.UP)) {
            ...
            if(direction==0){
                if(placeCheck == 0){
                    ...
                    placeMoss(level, placePos.north(), randomSource, treeConfiguration);
                    placeMoss(level, placePos.south(), randomSource, treeConfiguration);
    ...
    public boolean placeMoss(LevelAccessor level, BlockPos pos, RandomSource randomSource, FallenTreeConfiguration treeConfiguration) {
        if(level.isOutsideBuildHeight(pos)){
            return true;
        }
    ...
    public static boolean isReplaceable(LevelSimulatedReader reader, BlockPos pos) {
        return reader.isStateAtPosition(pos, FallenBlobTreeFeature::isReplaceableBlock);
    }
}