...
public class EnchantedBirchLeavesBlock extends LeavesBlock {
   public EnchantedBirchLeavesBlock(Properties p_273704_) {
      super(p_273704_);
   }
   ...
   public void animateTick(BlockState blockState, Level level, BlockPos blockPos, RandomSource randomSource) {
      ...
      if (randomSource.nextInt(10) == 0) {
         ...
         if (!isFaceFull(blockstate.getCollisionShape(level, blockpos), Direction.UP)) {
            if(RegionsUnexploredConfig.Client.TOGGLE_ENCHANTED_BIRCH_LEAVES_PARTICLE) {
               spawnParticleBelow(level, blockPos, randomSource, RuParticleTypes.ENCHANTED_BIRCH_LEAVES);
            }
         }
      }
   }
